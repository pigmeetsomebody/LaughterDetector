...
final class SystemAudioClassifier: NSObject {
    ...
    func startSoundClassification(subject: PassthroughSubject<SNClassificationResult, Error>,
        ...
        do {
            ...
            //let request = try SNClassifySoundRequest(mlModel: MLModel(contentsOf: URL(fileURLWithPath: "")))
    ...
    static func getAllPossibleLabels() throws -> Set<String> {
        //let request = SNClassifySoundRequest(mlModel: MLModel(contentsOf: URL(fileURLWithPath: "")))
        let version1 = SNClassifierIdentifier.version1
        let request = try SNClassifySoundRequest(classifierIdentifier: version1)
