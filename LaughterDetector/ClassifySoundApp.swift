...
class AppState: ObservableObject {
    ...
    func restartDetection(config: AppConfiguration) {
        ...
        self.detectionStates =
          ...
          .map { ($0, DetectionState(presenceThreshold: 0.3,
    ...
    static func advanceDetectionStates(_ oldStates: [(SoundIdentifier, DetectionState)],
        ...
        let confidenceForLabel = { (sound: SoundIdentifier) -> Double in
            ...
            if let classification = result.classification(for: label) {
                confidence = classification.confidence
                print("classification confidence : \(confidence)")
            } else {
                confidence = 0
            }
            return confidence
...
extension SNClassificationResult {
    func classification(for identifier: String) -> SNClassification? {
        
        return classifications.first {$0.identifier == identifier}
    }
}
